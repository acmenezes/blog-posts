...
Unfolding a little bit more on how the SCCs work we may ask: what inside Openshift takes care of checking if a Pod complies or not to it? The admission process in the API server will take care of that. OpenShift comes equipped with 8 predefined Security Context Constraints that you can list using the `oc get scc` command. For those who are not familiar with them, here they are:
...
|nonroot|nonroot provides all features of the restricted SCC but allows users to run with any non-root UID. The user must specify the UID or it must be specified on the by the manifest of the container runtime.
...
`oc apply -f my-deployment.yaml --as=unprivileged-user`
...
- my-admin-group
```
Other than that for the SCC commands check here to see the examples on the docs.
...
Troubleshooting SCCs can be quite challenging since it has so many ways to be configured. So, here are some things to keep in mind:
...
1. Ensures that pods cannot run as privileged.
...
2. Ensures that pods cannot mount host directory volumes.
...
3. Requires that a pod run as a user in a pre-allocated range of UIDs.
...
4. Requires that a pod run with a pre-allocated MCS label.
...
5. Allows pods to use any FSGroup.
...
6. Allows pods to use any supplemental group.
